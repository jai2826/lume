...
import { getAuthUserId, requireAuth } from "./lib/utils";
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const editorCode = Math.random()
      .toString(36)
      .substring(2, 12)
      .toUpperCase();
    const viewerCode = Math.random()
      .toString(36)
      .substring(2, 12)
      .toUpperCase();
...
export const join = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const editorMatch = await ctx.db
      .query("studios")
      .withIndex("by_editorInviteCode", (q) =>
        q.eq("editorInviteCode", cleanCode),
      )
      .first();
    ...
    if (editorMatch) {
      ...
      const viewerMatch = await ctx.db
        .query("studios")
        .withIndex("by_viewerInviteCode", (q) =>
          q.eq("viewerInviteCode", cleanCode),
        )
        .first();
